...
@compute @workgroup_size(1) fn generate(
  ...
  let y = id.x; // id is really a 3d coordinate, but we're only passing in one value to indicate what row we're on

  writeRow(y);
}

fn writeRow(y: u32) {
  for (var x: u32 = 0; x < dimensions.x; x++) {
    let i = x + (y * dimensions.x);
    let values = flattenQuad(generateCell(x, y, thickness));
